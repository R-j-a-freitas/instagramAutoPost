...
from instagram_poster.config import get_pollinations_api_key, get_media_root
...
st.title("Vídeo IA (Pollinations)")
st.caption("Gera um vídeo a partir da descrição de um post do Google Sheets usando Pollinations.ai.")
...
with col_model:
    vid_model = st.selectbox("Modelo", ["grok-video", "seedance", "veo", "wan"], index=0, 
                             help="grok-video (~6s fixo), seedance (~2s), veo (8s), wan (até 10s).")
with col_ratio:
...
if st.button("Gerar Vídeo com Pollinations", type="primary"):
    api_key = get_pollinations_api_key()
    if not api_key:
        st.warning("Configura POLLINATIONS_API_KEY na Configuração.")
...
if not st.session_state.video_ia_bytes:
    ...
    with col_music:
        ...
        st.caption("Nota: A mistura de música com o vídeo gerado requer processamento adicional (MoviePy).")
        audio_option = st.radio("Opção de Áudio", ["Sem música extra", "Biblioteca ambient", "Upload ficheiro"])
        ...
        if audio_option == "Biblioteca ambient":
            ...
            if tracks:
                ...
                sel_track = st.selectbox("Escolher faixa", list(track_options.keys()))
    ...
    if st.button("Publicar no Instagram como Reel", type="primary"):
        with st.spinner("A processar e publicar..."):
            try:
                ...
                if audio_path:
                    st.info(f"A misturar áudio: {Path(audio_path).name}...")
                    final_video_bytes = mix_video_with_audio(final_video_bytes, audio_path)
